...
public extension ObservableArray where Item: RealmSwift.Object{
    public func mapPrimaryKeys() -> SafeSignal<ObservableArrayEvent<PrimaryKey<Item>>>{
        return map { (event: ObservableArrayEvent<Item>) -> ObservableArrayEvent<PrimaryKey<Item>> in
            let mapped:[PrimaryKey<Item>] = event.source.map { item in
                return PrimaryKey<Item>(item: item)
            }
            let newEvent = ObservableArrayEvent(change: event.change, source: mapped)
            return newEvent
        }
    }
}
